...
def train_batch(
        ...
        output_dir: Path = typer.Option(Path("batch_output")),
...
@app.command()
def train_online(
        train_file: Path = typer.Option(Path("train.jsonl")),
        output_dir: Path = typer.Option(Path("online_ckpts")),
        epochs: int = typer.Option(3),
...
# --- 4) COMPARE MODELS ---
@app.command()
def compare(test_file: Path = typer.Option(Path("test.jsonl"))):
...
def start_server():
    ...
    def worker():
        for idx, (x, _) in enumerate(stream.iter_csv("train.csv", target="target", converters={"input": str})):
            if idx % 20 == 0:
