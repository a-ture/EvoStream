...
# Data Explorer
    ...
    df = load_jsonl(settings.DATA_DIR / f'{dataset}.jsonl')
    ...
    if st.button('De-ambigua ora'):
        if not seq.strip():
            ...
            with st.spinner('Chiamata al servizio...'):
                try:
                    resp = requests.post(settings.API_URL, json={'sequence': seq})
...
# Correzione
    ...
    df_test = load_jsonl(settings.TEST_FILE)
    # Pre-compute predictions
    if 'predicted' not in df_test.columns:
        df_test['predicted'] = df_test['input'].apply(
            lambda s: requests.post(settings.API_URL, json={'sequence': s}).json().get('resolved', '')
        )
    ...
    if st.button('Esporta correzioni'):
        save_path = settings.DATA_DIR / 'test_corrected.jsonl'
